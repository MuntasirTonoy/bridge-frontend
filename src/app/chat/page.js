...
export default function ChatPage() {
    ...
    if (status === "authenticated") {
      const fetchData = async () => {
        try {
          ...
          const hasConv = (userId) =>
            convsRes.data.some((cv) =>
              cv.participants.some((p) => p._id === userId),
            );
          const firstWithConv = usersRes.data.find(
            (u) =>
              hasConv(u._id) &&
              !archived.includes(u._id) &&
              !blocked.includes(u._id),
          );
        ...
        setActiveMessages((prev) => {
          if (
            prev.some(
              (m) =>
                m.id === message._id ||
                (m.isOptimistic && m.text === message.text),
            )
          ) {
            return prev.map((m) =>
              m.isOptimistic && m.text === message.text
          ...
          return [
            ...prev,
            {
              ...
              isRead:
                message.senderId === session?.user?.id ? message.isRead : true,
  ...
  const currentUser = {
    ...
    avatar:
      ...
      `https://api.dicebear.com/7.x/adventurer/svg?seed=${session.user.username}`,
  ...
  const formattedContacts = contacts
    .map((c) => {
      ...
      const conv = conversations.find((cv) =>
        cv.participants.some((p) => p._id === c._id),
      );
      ...
      return {
        ...
        avatar:
          c.profilePic ||
          `https://api.dicebear.com/7.x/adventurer/svg?seed=${c.username}`,
        isOnline,
        isTyping: !!typingUsers[c._id],
        about: c.bio || "",
        sharedMedia: [],
        extraMedia: 0,
        _lastMessageText:
          ...
          (conv?.lastMessage?.fileType === "image"
            ? "📷 Image"
            : conv?.lastMessage?.fileType === "pdf"
              ? "📄 PDF"
              : conv?.lastMessage?.fileType === "voice"
                ? "🎤 Voice"
                : "No messages yet"),
        _lastMessageTime: conv?.lastMessage
  ...
  const activeConversationData = {
    ...
    isTyping: !!typingUsers[activeContactId],
  };
  ...
  const handleEditMessage = async (messageId, newText) => {
    try {
      const res = await axios.put(`/chats/messages/${messageId}`, {
        text: newText,
      });
      setActiveMessages((prev) =>
        prev.map((m) =>
  ...
  const handleForwardMessage = async (recipientId) => {
    ...
    try {
      await handleSendMessage(
        ...
        recipientId,
      ); // Update handleSendMessage to accept targetId
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-bg-primary">
      ...
      <div
        className={`
        ${activeMobileView === "sidebar" ? "flex" : "hidden"}
        md:flex
      ...
      {forwardingMsg &&
        (() => {
          const forwardableContacts = formattedContacts.filter(
            (c) =>
              c._hasConversation &&
              !c._isBlocked &&
              !c.hasBlockedMe &&
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[300] flex items-center justify-center p-4">
              <div className="bg-bg-white w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl animate-pop-in flex flex-col max-h-[80vh]">
                <div className="p-6 border-b border-border-light flex items-center justify-between">
                  ...
                  <button
                    className="p-2 rounded-full hover:bg-bg-primary text-text-muted"
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-1">
                  {forwardableContacts.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-text-muted gap-3">
                      ...
                      <p className="text-[0.78rem] text-center px-6">
                        Start a conversation with someone else first to forward
                        messages.
                      </p>
                    </div>
                  ) : (
                    forwardableContacts.map((c) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[0.95rem] font-semibold text-text-primary truncate">
                            {c.name}
                          </p>
                          ...
                          {c._lastMessageText &&
                            c._lastMessageText !== "No messages yet" && (
                              <p className="text-[0.72rem] text-text-muted truncate mt-0.5 opacity-70">
                                {c._lastMessageText}
                              </p>
                            )}
                        </div>
